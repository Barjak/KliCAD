/*
 * KliCAD gerbview kiface binding registration.  See header for rationale.
 */
...
bool s_gerbview_bindings_registered = false;
...
{
    if( s_gerbview_bindings_registered )
        return;

    if( !Py_IsInitialized() )
    {
        wxLogTrace( traceApi,
                    "KliCAD: skipping gerbview binding registration — "
    ...
    try
    {
        ...
        s_gerbview_bindings_registered = true;
    }
    catch( const py::error_already_set& e )
    {
        wxLogTrace( traceApi,
                    "KliCAD: gerbview binding registration raised Python "
                    "exception: %s", e.what() );
    }
    catch( const std::exception& e )
    {
        wxLogTrace( traceApi,
                    "KliCAD: gerbview binding registration failed: %s",
                    e.what() );
    }
}
