/*
 * KliCAD cvpcb kiface binding registration.  See header for rationale.
 */
...
bool s_cvpcb_bindings_registered = false;
...
{
    if( s_cvpcb_bindings_registered )
        return;

    if( !Py_IsInitialized() )
    {
        wxLogTrace( traceApi,
                    "KliCAD: skipping cvpcb binding registration — "
    ...
    try
    {
        ...
        s_cvpcb_bindings_registered = true;
    }
    catch( const py::error_already_set& e )
    {
        wxLogTrace( traceApi,
                    "KliCAD: cvpcb binding registration raised Python "
                    "exception: %s", e.what() );
    }
    catch( const std::exception& e )
    {
        wxLogTrace( traceApi,
                    "KliCAD: cvpcb binding registration failed: %s",
                    e.what() );
    }
}
